...
const BibleReadingApp = () => {
  ...
  const isReadingComplete = (dateKey, session, type, reference) => {
    return completedReadings[dateKey]?.[session]?.[type]?.includes(reference) || false;
  };

  // Toggle reading completion
  const toggleReadingComplete = (dateKey, session, type, reference, e) => {
    ...
    setCompletedReadings(prev => {
      ...
      if (!updated[dateKey][session]) updated[dateKey][session] = {};
      if (!updated[dateKey][session][type]) updated[dateKey][session][type] = [];

      const readings = updated[dateKey][session][type];
      const index = readings.indexOf(reference);

      if (index > -1) {
        // Remove if already complete (un-mark)
        readings.splice(index, 1);
      } else {
        // Add if not complete (mark as read)
  ...
  // Mark reading as complete when link is clicked
  const handleReadingClick = (dateKey, session, type, reference) => {
    if (!isReadingComplete(dateKey, session, type, reference)) {
      setCompletedReadings(prev => {
        const updated = { ...prev };
        ...
        if (!updated[dateKey][session][type].includes(reference)) {
          updated[dateKey][session][type].push(reference);
        }
  ...
  const getSessionProgress = (dateKey, session, sessionData) => {
    if (!sessionData) return { completed: 0, total: 0 };
    
    const totalReadings = sessionData.psalms.length + sessionData.lesson.length;
    const completedPsalms = sessionData.psalms.filter(p => 
      isReadingComplete(dateKey, session, 'psalms', p.reference)
    ).length;
    const completedLessons = sessionData.lesson.filter(l => 
      isReadingComplete(dateKey, session, 'lesson', l.reference)
    ).length;
    
    return {
      completed: completedPsalms + completedLessons,
      total: totalReadings
    };
  };
